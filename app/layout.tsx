...
export default function RootLayout({children}:{children:React.ReactNode}){return <html lang="en"><body><div className="app-shell"><aside className="sidebar"><div className="brand"><Image className="brand-logo" src="/mane-masala-logo.jpeg" alt="Mane Masala" width={52} height={52}/><div><div className="brand-name">Mane Masala</div><div className="brand-subtitle">Business System</div></div></div><nav aria-label="Main navigation">{navigation.map(([label,href])=><Link className="nav-item" href={href} key={href}>{label}</Link>)}</nav></aside><main className="main-content"><header className="topbar"><div><div className="eyebrow">Mane Masala</div><div className="page-title">Business Management System</div></div><div className="topbar-actions"><ThemePicker/><div className="foundation-badge">Live database</div></div></header>{children}</main></div></body></html>}
