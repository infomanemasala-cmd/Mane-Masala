...
const HIDDEN_COLUMNS = new Set(['id', 'created_at', 'updated_at', 'category_id', 'base_unit_id', 'purchase_unit_id', 'selling_unit_id', 'customer_id', 'sub_agent_id', 'end_customer_customer_id'])
const SEARCH_COLUMNS: Record<string, string[]> = {
  items: ['item_code', 'business_code', 'name', 'item_type', 'subcategory', 'product_family', 'notes'], suppliers: ['business_code', 'business_name', 'contact_person', 'supplier_type', 'phone', 'whatsapp', 'upi_id', 'gst_number', 'email', 'notes'], customers: ['business_code', 'name', 'customer_type', 'phone', 'whatsapp', 'email', 'gst_number', 'notes'],
  units: ['code', 'name', 'symbol'], categories: ['code', 'name'], sub_agents: ['business_code', 'name', 'phone', 'whatsapp', 'notes'], purchases: ['business_code', 'supplier_invoice_number', 'system_reference', 'purchase_source', 'financial_status', 'workflow_status', 'notes'], inventory_transactions: ['business_code', 'transaction_type', 'reference_type', 'notes'], v_inventory_current: ['business_code', 'name'], recipes: ['business_code', 'name', 'status', 'notes'], production_batches: ['business_code', 'status', 'wife_approval_status', 'notes'], orders: ['business_code', 'source', 'status', 'order_party_type', 'end_customer_name', 'end_customer_phone', 'requests', 'notes'], sales: ['business_code', 'status', 'notes'], invoices: ['business_code', 'invoice_number', 'financial_year', 'status', 'notes'], supplier_payments: ['business_code', 'payment_method', 'upi_reference', 'notes'], customer_payments: ['business_code', 'payment_method', 'upi_reference', 'notes'], dispatches: ['business_code', 'dispatch_date', 'dispatch_method', 'status', 'notes'], stock_outs: ['business_code', 'stock_out_date', 'reason', 'notes'], stock_adjustments: ['business_code', 'adjustment_date', 'reason', 'notes'],
}
const DEFAULT_SORT: Record<string, string> = { items: 'item_code', units: 'code', categories: 'code', suppliers: 'business_name', customers: 'name', sub_agents: 'name', purchases: 'purchase_date', v_inventory_current: 'business_code', recipes: 'name', production_batches: 'production_date', orders: 'order_date', dispatches: 'dispatch_date', sales: 'sale_date', invoices: 'invoice_date', supplier_payments: 'payment_date', customer_payments: 'payment_date', stock_outs: 'stock_out_date', stock_adjustments: 'adjustment_date', v_supplier_outstanding: 'business_name', v_customer_outstanding: 'name' }
function label(column: string) { if (column === 'item_code') return 'Item Code'; return column.replaceAll('_', ' ').replace(/\b\w/g, (letter) => letter.toUpperCase()) }
