...
const SEARCH_COLUMNS: Record<string, string[]> = {
  ...
  inventory_transactions: ['business_code', 'transaction_type', 'reference_type', 'notes'], v_inventory_current: ['business_code', 'name'],
  recipes: ['business_code', 'name', 'status', 'notes'], production_batches: ['business_code', 'status', 'wife_approval_status', 'notes'],
  orders: ['business_code', 'source', 'status', 'requests', 'notes'], sales: ['business_code', 'status', 'notes'],
  invoices: ['business_code', 'invoice_number', 'financial_year', 'status', 'notes'], supplier_payments: ['business_code', 'payment_method', 'upi_reference', 'notes'],
  customer_payments: ['business_code', 'payment_method', 'upi_reference', 'notes'], v_supplier_outstanding: ['business_code', 'business_name'], v_customer_outstanding: ['business_code', 'name'],
}
...
export default function DataTable({ table, refreshToken = 0 }: { table: string; refreshToken?: number }) {
  ...
  return <div className="data-table">
    <div className="table-toolbar"><input className="table-search" value={search} onChange={(event) => changeSearch(event.target.value)} placeholder="Search…" aria-label={`Search ${table}`} /><div className="table-toolbar-right"><span className="table-count">{total} record{total === 1 ? '' : 's'}</span><select value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1) }} aria-label="Rows per page">{PAGE_SIZES.map((size) => <option key={size} value={size}>{size} per page</option>)}</select></div></div>
    <div className="table-wrap"><table><thead><tr>{columns.map((column) => <th key={column}><button className="table-sort" type="button" onClick={() => chooseSort(column)}>{label(column)} {sort === column ? (ascending ? '↑' : '↓') : '↕'}</button></th>)}</tr></thead><tbody>{loading ? <tr><td colSpan={Math.max(columns.length, 1)} className="table-message">Loading…</td></tr> : !rows.length ? <tr><td colSpan={Math.max(columns.length, 1)} className="table-message">No records found.</td></tr> : rows.map((row, index) => <tr key={String(row.id ?? `${page}-${index}`)}>{columns.map((column) => <td key={column}>{displayValue(row[column])}</td>)}</tr>)}</tbody></table></div>
    <div className="table-pagination"><span>Showing {start}–{end} of {total}</span><div><button type="button" onClick={() => setPage((value) => Math.max(1, value - 1))} disabled={page <= 1}>Previous</button><span>Page {page} of {totalPages}</span><button type="button" onClick={() => setPage((value) => Math.min(totalPages, value + 1))} disabled={page >= totalPages}>Next</button></div></div>
  </div>
}
