...
export default function InventoryConsoleV3() {
  ...
  const resetForm = () => { setOpen(false); setItemId(''); setUnitId(''); setQuantity(''); setReason('damage'); setNotes(''); setSaving(false) }
  const openAction = () => { setMessage(''); setError(false); setOpen(true) }
  ...
  const submit = async () => {
    ...
    setError(false); setMessage(kind === 'out' ? 'Stock Out recorded.' : 'Stock Adjustment recorded.'); setRefreshToken(v => v + 1); resetForm()
  }
