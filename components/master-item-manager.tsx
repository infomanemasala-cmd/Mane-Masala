...
export default function MasterItemManager() {
  ...
  const [selected, setSelected] = useState<string[]>([]), [editing, setEditing] = useState<Row | null>(null), [message, setMessage] = useState(''), [error, setError] = useState(''), [busy, setBusy] = useState(false)
  ...
  const archive = async () => {
    if (!selected.length) return; if (!window.confirm(`Archive ${selected.length} selected record${selected.length === 1 ? '' : 's'}? They will remain in history.`)) return
    setBusy(true); setMessage(''); setError('')
    for (const id of selected) { const { error: e } = await db().rpc('archive_master_record', { p_table: 'items', p_id: id, p_reason: 'Archived from Master Data' }); if (e) { setError(e.message); setBusy(false); return } }
    setSelected([]); setBusy(false); setMessage('Archived successfully.'); await load()
  }
  ...
  return <div className="master-manager">
    ...
    {message && <p className="form-status form-status-success">{message}</p>}{error && <p className="form-status form-status-error">{error}</p>}
