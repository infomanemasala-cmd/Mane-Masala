...
const itemTypes = [['raw_material', 'Raw material'], ['intermediate', 'Intermediate'], ['finished_product', 'Finished product'], ['purchased_finished_product', 'Purchased finished product']]
...
function MasterForm({ table, onSaved }: { table: MasterTable; onSaved: () => void }) {
  ...
  const [units, setUnits] = useState<Row[]>([]), [categories, setCategories] = useState<Row[]>([]), [customers, setCustomers] = useState<Row[]>([])
  useEffect(() => {
  ...
  return <form className="console-form" onSubmit={save}>
    ...
    {table === 'suppliers' && <><Field label="Business / Store name"><input name="business_name" required placeholder="Ganesh Stores" /></Field><Field label="Supplier type"><select name="supplier_type" defaultValue="wholesaler">{supplierTypes.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></Field><Field label="Phone"><input name="phone" /></Field><Field label="WhatsApp"><input name="whatsapp" /></Field><Field label="UPI ID"><input name="upi_id" /></Field><Field label="Preferred payment"><select name="preferred_payment_method" defaultValue=""><option value="">Not specified</option><option value="cash">Cash</option><option value="upi">UPI</option><option value="other">Other</option></select></Field><Field label="Notes"><input name="notes" /></Field></>}
    {table === 'customers' && <><Field label="Customer name"><input name="name" required placeholder="Customer name" /></Field><Field label="Customer type"><select name="customer_type" defaultValue="individual">{customerTypes.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></Field><Field label="Phone"><input name="phone" /></Field><Field label="WhatsApp"><input name="whatsapp" /></Field><Field label="Notes"><input name="notes" /></Field></>}
    {table === 'sub_agents' && <><Field label="Existing sub-agent customer"><select name="customer_id" required><option value="">Select customer</option>{customers.map((customer) => <option key={String(customer.id)} value={String(customer.id)}>{String(customer.name)}</option>)}</select></Field><Field label="Sub-agent name"><input name="name" required /></Field><Field label="Phone"><input name="phone" /></Field><Field label="WhatsApp"><input name="whatsapp" /></Field><Field label="Notes"><input name="notes" /></Field></>}
    {table === 'items' && <><Field label="Item name"><input name="name" required placeholder="Chilli Powder" /></Field><Field label="Item type"><select name="item_type" defaultValue="raw_material">{itemTypes.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></Field><Field label="Category"><select name="category_id" defaultValue=""><option value="">No category</option>{categories.map((category) => <option key={String(category.id)} value={String(category.id)}>{String(category.name)}</option>)}</select></Field><Field label="Base / purchase unit"><select name="base_unit_id" required defaultValue=""><option value="">Select unit</option>{units.map((unit) => <option key={String(unit.id)} value={String(unit.id)}>{String(unit.name)} ({String(unit.symbol)})</option>)}</select></Field><Field label="Selling unit"><select name="selling_unit_id" defaultValue=""><option value="">Same as base / not applicable</option>{units.map((unit) => <option key={String(unit.id)} value={String(unit.id)}>{String(unit.name)} ({String(unit.symbol)})</option>)}</select></Field><Field label="Minimum stock"><input name="minimum_stock" type="number" min="0" step="0.001" defaultValue="0" /></Field><Field label="Perishable"><input name="is_perishable" type="checkbox" /></Field><Field label="Track expiry"><input name="expiry_tracking_enabled" type="checkbox" /></Field><Field label="Notes"><input name="notes" /></Field></>}
    <SubmitButton busy={busy}>Save</SubmitButton>{message && <p className="form-status form-status-success">{message}</p>}{error && <p className="form-status form-status-error">{error}</p>}
  </form>
}

export default function MasterManager({ table }: { table: MasterTable }) {
  const [refreshToken, setRefreshToken] = useState(0)
  return <div className="master-manager"><div className="master-create"><h3>Add {table.replaceAll('_', ' ')}</h3><MasterForm table={table} onSaved={() => setRefreshToken((value) => value + 1)} /></div><div className="master-list"><DataTable table={table} refreshToken={refreshToken} /></div></div>
}
