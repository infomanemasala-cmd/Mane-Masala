...
export default function ProductionConsoleV2() {
  ...
  const complete = async () => {
    if (!selected) return
    const consumptions = plan.filter(x => Number(x.actual) > 0).map(x => ({ ingredient_item_id: x.itemId, actual_quantity: Number(x.actual), unit_id: x.unitId }))
    ...
    const { error: e } = await db().rpc('complete_production', { p_production_batch_id: batchId, p_consumptions: consumptions, p_outputs: [{ output_item_id: selected.output_item_id, quantity: Number(output), unit_id: selected.output_unit_id }] })
    if (e) { setError(true); setMessage(e.message); return }
    setError(false); setMessage('Production completed. FIFO ingredient consumption and output stock were recorded.'); await load(); setRefreshToken(v => v + 1); setOpen(false)
  }

  return <section className="page-panel">
    <div className="section-label">Mane Masala</div>
    <div className="master-header"><div><h1>Production</h1><p className="page-intro">Review production by batch, item, date or status. Select a batch to approve, start and complete its order-linked production flow.</p></div><button className="primary-button" type="button" onClick={() => { setOpen(true); setMessage(''); setError(false) }}>+ Review Production</button></div>
