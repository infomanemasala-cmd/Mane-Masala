  ...
  return <section className="page-panel">
    ...
    {open && <Modal title={step === 'entry' ? 'Create Purchase' : step === 'receipt' ? 'Receive & Inspect Purchase' : step === 'attach' ? 'Attach Invoice / Slip' : step === 'payment' ? 'Payment Details' : 'Purchase Complete'} close={close}>
      {step === 'entry' && <>
        ...
        <div className="purchase-payment-choice"><strong>Payment at this purchase</strong><div className="master-tabs"><button type="button" className={paymentMode==='now'?'tab-active':''} onClick={() => setPaymentMode('now')}>Paid / payment entered now</button><button type="button" className={paymentMode==='later'?'tab-active':''} onClick={() => setPaymentMode('later')}>Record payment later</button><button type="button" className={paymentMode==='credit'?'tab-active':''} onClick={() => setPaymentMode('credit')}>Credit</button></div><small>Payments are separate transactions. You can record an advance, bill payment, or multiple payments here; otherwise enter them later from Payments.</small></div>
      ...
      {step === 'payment' && <><div className="purchase-step"><span>4</span><div><strong>Payment / credit</strong><small>Record what actually happened, including an advance paid before the bill date. Payment date is the real offline payment date.</small></div></div><div className="purchase-payment-choice"><strong>How was this purchase settled?</strong><div className="master-tabs"><button type="button" className={paymentMode==='now'?'tab-active':''} onClick={() => setPaymentMode('now')}>Record payment now</button><button type="button" className={paymentMode==='later'?'tab-active':''} onClick={() => setPaymentMode('later')}>Record later</button><button type="button" className={paymentMode==='credit'?'tab-active':''} onClick={() => setPaymentMode('credit')}>Credit / unpaid</button></div></div>{paymentMode==='now'&&<><h3>Payments</h3>{payments.map((p,i)=><div className="purchase-line payment-line" key={i}><Field label="Payment type"><select value={p.stage} onChange={e => setPayments(v=>v.map((x,n)=>n===i?{...x,stage:e.target.value}:x))}><option value="advance">Advance toward this purchase</option><option value="bill">Bill payment</option></select></Field><CurrencyField label="Amount" value={p.amount} onChange={v=>setPayments(x=>x.map((a,n)=>n===i?{...a,amount:v}:a))}/><Field label="Actual payment date"><input type="date" value={p.date} onChange={e=>setPayments(v=>v.map((x,n)=>n===i?{...x,date:e.target.value}:x))}/></Field><Field label="Method"><select value={p.method} onChange={e=>setPayments(v=>v.map((x,n)=>n===i?{...x,method:e.target.value}:x))}><option value="upi">UPI</option><option value="cash">Cash</option><option value="bank_transfer">Bank transfer</option><option value="other">Other</option></select></Field><Field label="UPI / payment reference"><input value={p.reference} onChange={e=>setPayments(v=>v.map((x,n)=>n===i?{...x,reference:e.target.value}:x))}/></Field><Field label="Notes"><input value={p.notes} onChange={e=>setPayments(v=>v.map((x,n)=>n===i?{...x,notes:e.target.value}:x))}/></Field><button className="line-remove" type="button" onClick={() => setPayments(v=>v.length===1?v:v.filter((_,n)=>n!==i))} disabled={payments.length===1}>Remove</button></div>)}<button className="add-row-button" type="button" onClick={()=>setPayments(v=>[...v,blankPayment()])}>＋ Add another payment</button><p className="purchase-total"><strong>Total payment entered: ₹ {paymentTotal.toFixed(2)}</strong></p></>}{paymentMode==='later'&&<p className="muted">No payment is recorded now. The purchase can remain unpaid/credit until the actual offline payment is entered later in Payments.</p>}{paymentMode==='credit'&&<p className="muted">No payment is recorded. Supplier outstanding will remain until a later payment or correction.</p>}<Status message={message} error={error}/><div className="purchase-actions"><button className="secondary-button" type="button" onClick={close}>Cancel</button><button className="primary-button" type="button" onClick={recordPayments}>{paymentMode==='now'?'Save payment details & finish':'Finish purchase entry'}</button></div></>}
