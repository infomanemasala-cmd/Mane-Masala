create or replace function public.save_actuals_as_recipe_version(p_production_batch_id uuid,p_consumptions jsonb,p_actual_output numeric) returns jsonb language plpgsql security definer set search_path to '' as $function$
declare v_user uuid:=auth.uid(); b record; oldrv record; x record; v_next int; v_new uuid; v_qty numeric; v_scale numeric; v_seq int:=1;
begin
...
if b.status not in ('planned','in_progress') or b.wife_approval_status<>'approved' then raise exception 'Recipe learning can only be saved for an approved production batch'; end if;
...
insert into public.recipe_versions(recipe_id,version_number,expected_output_quantity,output_unit_id,notes,status,effective_from,created_by) values(oldrv.recipe_id,v_next,oldrv.expected_output_quantity,oldrv.output_unit_id,coalesce(oldrv.notes,'')||' | learned from actual production batch','active',current_date,v_user) returning id into v_new;
for x in select ingredient_item_id,unit_id,sum(actual_quantity) actual_quantity from jsonb_to_recordset(p_consumptions) as z(ingredient_item_id uuid,actual_quantity numeric,unit_id uuid) where actual_quantity>0 group by ingredient_item_id,unit_id order by ingredient_item_id loop
 v_scale:=oldrv.expected_output_quantity/p_actual_output; v_qty:=x.actual_quantity*v_scale;
 insert into public.recipe_lines(recipe_version_id,ingredient_item_id,quantity,unit_id,sequence_number,notes) values(v_new,x.ingredient_item_id,v_qty,x.unit_id,v_seq,'Learned from actual production'); v_seq:=v_seq+1;
end loop;
...
revoke all on function public.save_actuals_as_recipe_version(uuid,jsonb,numeric) from public,anon;
grant execute on function public.save_actuals_as_recipe_version(uuid,jsonb,numeric) to authenticated;
