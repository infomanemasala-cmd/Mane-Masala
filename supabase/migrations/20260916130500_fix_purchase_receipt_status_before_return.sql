-- Receiving is not the same as returning.
-- A rejected/damaged quantity is recorded during inspection but remains pending
-- until the user explicitly records the supplier return.
...
begin
 ...
 for v_line in select * from jsonb_array_elements(p_lines) loop
  ...
  values(public.gen_random_uuid(),v_receipt_id,v_purchase_line.id,v_received,v_accepted,v_rejected,v_replacement,v_purchase_line.unit_id,
         coalesce((v_line->>'received_batch_date')::date,coalesce(p_received_at,now())::date),(v_line->>'expiry_date')::date,(v_line->>'best_before_date')::date,v_line->>'notes');
  ...
  if v_accepted>0 then
   ...
   values(v_batch_id,v_purchase_line.item_id,'purchase_receipt',v_receipt_id,v_purchase_line.id,coalesce(p_received_at,now())::date,v_accepted,v_accepted,v_purchase_line.unit_id,v_purchase_line.unit_rate,(v_line->>'expiry_date')::date,(v_line->>'best_before_date')::date,'active');
